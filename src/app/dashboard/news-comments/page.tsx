...
const AllCommentsPage = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      setLoading(true);
      try {
        const response = await axios.get('https://hltback.parfumetrika.ru/news/comments/all', {
          params: {
            page,
            limit,
          },
  ...
  const handleDelete = async (news_id: string, commentId: string) => {
    ...
    try {
      await axios.delete(`https://hltback.parfumetrika.ru/news/comments/${commentId}`, {
        headers: {
